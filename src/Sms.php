...
class Sms
{
    ...
    public function verify(string $receptor, string $template, string $token1, ?string $token2 = null, ?string $token3 = null)
    {
        $driver = $this->getDriverInstance();
        $driver->to($receptor);
        return $driver->verify($template, $token1, $token2, $token3);
    }
